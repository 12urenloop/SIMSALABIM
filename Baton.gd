...
func _get_progress():
	return progress/get_track_distance()
...
func _process(delta):
	...
	queue_redraw()
	var previous_offset = progress
	
	temp_slow = lerp(temp_slow, 1.0, .1)
	
	progress += ((speed + speed_offset) * temp_slow)
	if progress < previous_offset:
		laps += 1
	...
	if previous_offset < home_base and progress > home_base:
		temp_slow = .3
